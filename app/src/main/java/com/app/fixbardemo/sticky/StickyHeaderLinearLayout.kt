package com.app.fixbardemo.sticky
...
class StickyHeaderLinearLayoutManager @JvmOverloads constructor(
    context: Context,
    private val stickyHeaderCallbacks: StickyHeaderCallbacks? = null,
    ...
    private fun setAdapter(newAdapter: RecyclerView.Adapter<*>?) {
        ...
        if (newAdapter is RecyclerView.Adapter) {
            adapter = newAdapter
    ...
    override fun scrollVerticallyBy(dy: Int, recycler: RecyclerView.Recycler, state: RecyclerView.State?): Int {
        ...
        if (scrolled != 0) {
//            Log.d("wuguoqing", "scrollVerticallyBy")
    ...
    override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        if (!state.isPreLayout) {
            Log.d("wuguoqing", "onLayoutChildren")
    ...
    fun findFirstVisibleItemPositionWithoutHeader(): Int {
        stickyHeader?.let(this::detachView)
        val result = findFirstVisibleItemPosition()
        stickyHeader?.let(this::attachView)
        return result
    }

    fun findLastVisibleItemPositionWithoutHeader(): Int {
        stickyHeader?.let(this::detachView)
        val result = findLastVisibleItemPosition()
        stickyHeader?.let(this::attachView)
        return result
    }
    ...
    private fun updateStickyHeader(recycler: RecyclerView.Recycler, layout: Boolean) {
        ...
        if (headerCount > 0 && childCount > 0) {
            ...
            if (anchorView != null && anchorPos != -1) {
                ...
                Log.d("wuguoqing", "anchorPos=$anchorPos, headerPos=$headerPos, nextHeaderPos=$nextHeaderPos")
...
//                        Log.d("wuguoqing", "getY: ${it.translationY}")
                    }
                    return
                }
            }
    ...
    private fun createStickyHeader(recycler: RecyclerView.Recycler, position: Int) {
        ...
        Log.d("wuguoqing", "createStickyHeader: position=$position, stickyHeader=$stickyHeader")
    ...
    private fun bindStickyHeader(recycler: RecyclerView.Recycler, stickyHeader: View, position: Int) {
        ...
        Log.d("wuguoqing", "bindStickyHeader: position=$position, stickyHeader=$stickyHeader")
        ...
        if (scrollPosition != RecyclerView.NO_POSITION) {
            stickyHeader.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
                override fun onGlobalLayout() {
                    ...
                    if (scrollPosition != RecyclerView.NO_POSITION) {
                        Log.d("wuguoqing", "scrollPosition = $scrollPosition, scrollOffset = $scrollOffset")
    ...
    private fun scrapStickyHeader(recycler: RecyclerView.Recycler?) {
        ...
        Log.d("wuguoqing", "scrapStickyHeader: stickyHeaderPosition=$stickyHeaderPosition")
    ...
    private fun RecyclerView.Adapter<*>.isStickyHeader(position: Int): Boolean? {
        return stickyHeaderCallbacks?.isStickyHeader(position)
    }

    private fun RecyclerView.Adapter<*>.setupStickyHeaderView(stickyHeader: View) {
        stickyHeaderCallbacks?.setupStickyHeaderView(stickyHeader)
    }

    private fun RecyclerView.Adapter<*>.teardownStickyHeaderView(stickyHeader: View) {
        stickyHeaderCallbacks?.teardownStickyHeaderView(stickyHeader)
    }
